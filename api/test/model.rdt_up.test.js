...
import { RdtUsers, RdtItems, RdtUps } from '../server/models';
import { generateUUID } from '../server/util';

describe("The Server's RdtUps model", () => {
  let database, user, item, upvoteId = generateUUID(), itemId = generateUUID(), thingId = generateUUID(), downvote;
  ...
  before(async () => {
    ...
    await RdtUps.create({ uid: upvoteId, userRef: user.uid, rdtRef: item.uid });
    downvote = await RdtUps.findOne({ where: { userRef: user.uid, rdtRef: item.uid } });
  })
  ...
  describe('# RdtUps.findOne function', () => {
    ...
    it("the downvote's attributes should have the correct values", () => {
      chai.expect(downvote.uid).to.equal(upvoteId);
